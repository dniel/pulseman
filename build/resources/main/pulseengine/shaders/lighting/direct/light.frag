...
void main()
{
    ...
    // Set fragment color (step() keeps edgeCount and ambientColor uniforms alive to prevent
    // GLSL compiler from stripping them, which would cause warnings in DirectLightingSystem)
    fragColor = vec4(finalColor, step(0.0, float(edgeCount) + ambientColor.a + 2.0));
}