 ...
 * BFS flood-fill from Pac-Man's start position to convert any unreachable walkable tiles to walls,
...
object Maze {
    ...
    /** Pac-Man's starting grid column. */
    const val PAC_START_X = 14
    /** Pac-Man's starting grid row. */
    const val PAC_START_Y = 21
    ...
    /**
     * Whether Pac-Man can walk on the given tile.
    ...
    /** Returns walkable directions for Pac-Man from the given grid position. */
    ...
    /**
     * BFS flood-fill from Pac-Man's start position. Any walkable tile not reachable
    ...
    private fun sealUnreachableTiles(g: Array<IntArray>) {
        ...
        queue.add(PAC_START_X to PAC_START_Y)
        visited[PAC_START_Y][PAC_START_X] = true
