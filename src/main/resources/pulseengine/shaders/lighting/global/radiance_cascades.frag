...
HitResult raymarch(vec2 rayPos, vec2 rayDir, float rayLen)
{
    ...
    {
        // sampler2D cannot be used in ternary on Mesa/Intel/AMD drivers
        float stepSize;
        if (space == LOCAL)
            stepSize = max(0.0, textureLod(localSdfTex, rayPos * invSdfRes, 0.0).r);
        else
            stepSize = max(0.0, textureLod(globalSdfTex, rayPos * invSdfRes, 0.0).r);
...
vec4 sampleScene(float space, vec2 originPos, vec2 hitPosUv, vec2 rayDir)
{
    ...
    vec4 metadata;
    if (space == LOCAL) {
        color = texture(localSceneTex, hitPosUv);
        metadata = texture(localMetadataTex, hitPosUv);
    } else {
        color = texture(globalSceneTex, hitPosUv);
        metadata = texture(globalMetadataTex, hitPosUv);
    }
